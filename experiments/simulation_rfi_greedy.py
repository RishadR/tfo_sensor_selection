...
def run_simulation_rfi_greedy(
    model_name: ModelName = "mlp",
    generator_name: str = "arf",
    generator_model_path: Path | None = Path("models/simulation_arf.pkl"),  # Reuse fitted model
    rfi_split: RFISplit = "test",
    seed: list[int] | None = None,
    synthesize_count: int = 1,
    n_job_synthesize: int = 1,
    record_rfi_csv: bool = True,
    rfi_output_path: str | Path = "results/simulation_rfi_greedy_results.csv",
...
if __name__ == "__main__":
    result = run_simulation_rfi_greedy(
        model_name="mlp",
        generator_name="arf",
        rfi_split="test",
        synthesize_count=30,
